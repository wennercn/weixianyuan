...
Ext.define("WXY.dangers.Form", {
    ...
    initComponent: function () {
        ...
        var catalogCombo = Ext.create('Ext.form.ComboBox', {
            store: "dangers-catalog", //store:Catalog Window 
            name: "dangerclassid",
            queryMode: 'local', //local本地读取 remote远程读取
            displayField: 'classname',
            valueField: 'dangerclassid',
            allowBlank: false,
            editable: false,
            flex: 1
        });
        ...
        Ext.apply(this, {
            items: [
				...
				{ fieldLabel: "标题", name: "dangername", allowBlank: false, afterLabelTextTpl: req },
				{ xtype: "textarea", fieldLabel: "内容", name: "description", allowBlank: false, anchor: "100% -90", afterLabelTextTpl: req },
    ...
    initMain: function (config) {
        ...
        if (config.record) {
            ...
            this.win.setTitle("修改 " + this.record.get("dangername") + " 内容");
    ...
    showCataLog: function () {
        if (!this.win.catalog) {
            this.win.catalog = this.win.add(
				Ext.create("WXY.dangers.Catalog")
			);
        }
    ...
    save: function () {
        ...
        */
        ...
        Ext.Ajax.request({
            url: this.win.wsUrl + "AttachDanger", //(this.record ? "UpdateDanger" : "AddDanger") ,
            params: $params(fv, 'danger'),
            success: this._save,
            failure: $failure,
            scope: this
        });
