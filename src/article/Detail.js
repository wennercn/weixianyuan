Ext.define("WXY.article.Detail" , {
	...
	initMain: function(config){
		...
		this.win.setTitle("查看 "+this.record.get("title")+" 详细信息");
	...
	getTemplate: function(){
		var t = new Ext.Template([
			'<div style="padding:15px;text-align:center">',
				'<h3 style="font:bold 18px arial;padding:10px 0">{title}</h3>' , 
				'<div style="font:normal 12px arial;line-height:200%;padding:10px 0;border-top:#ccc solid 1px;text-align:left">{detail}</div>' , 
			'</div>',
		]);		
		return t;	
	}
