Ext.define('WXY.dangers.List', {
    ...
    */
    ...
    columns: [
		...
		{ text: "标题", dataIndex: 'dangername', flex: 1, renderer: function (v, td, r) {
		    td.style = 'white-space:normal;';
		    return '<p style="font-weight:bold">' + v + "</p><div style='color:#999;line-height:150%'>" + r.get("description").substr(0, 50) + "</div>"
		}
		},
    //{text:"内容" , dataIndex:'detail' , flex:1} , 
		{text: '分类', dataIndex: 'classname', align: 'center' }
	],
    ...
    */
    load: function (params) {
        ...
        this.store.load({
            url: this.win.wsUrl + "GetDangerByClssID",
            params: pa,
    ...
    getParams: function () {
        var pa = {};
        pa.classname = this.win.moduleConfig.type;
        pa.classid = this.win.moduleConfig.id;

        //还没到这
        //alert(win.moduleConfig.id);
...
execDelete: function (result) {
    ...
    MB.loading("删除信息");
    Ext.Ajax.request({
        url: this.win.wsUrl + "DeleteDanger",
        params: { dangerid: this.forDeleteRecord.get("dangerid") },
