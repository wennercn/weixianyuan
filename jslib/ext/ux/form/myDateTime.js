 ...
 */
Ext.define('Ext.ux.form.myDateTime', {
...
//    dateTimeFormat: 'Y-m-d H:i:s',
    ...
    initComponent: function(){
        ...
        me.timeField = Ext.create('Ext.form.field.Text', Ext.apply({
        ...
        if (date){
            if (time && time.length == 4){
				...
				if (this.mustDate) {
					...
					value = Ext.Date.format(this.dateField.getValue() , "Y-m-d")+" 后告";
				}
            }
        }
        return value || "";
...
//        return value ? Ext.Date.format(value, this.dateTimeFormat) : null;
        ...
        var me = this
            ,format = me.getFormat()
            ,value = me.getValue();
        return value ? Ext.Date.format(value, "Y-m-d H:i:s") : null;
    },

    setValue: function(value){
		var d;
        if (Ext.isString(value)){
            d = Ext.Date.parse(value, this.getFormat()); //this.dateTimeFormat
			if (!d){
				d = Ext.Date.parse(value , "Y-m-d H:i:s");
			}
			if (!d){
				var vs = value.split(" ");
				var date = vs[0];
				var time = vs.length > 1 ? vs[1] : "";
				this.dateField.setValue(date);
				time = time == "后告" ? "" : time;
				if (time.indexOf(":")>-1){
					time = time.split(":");
					time = time[0]+(time.length>1 ? time[1] : "00")
				}
		...
		try{
			this.timeField.setValue(value ? Ext.Date.format(value , "Hi") : "");
		}catch(e){
